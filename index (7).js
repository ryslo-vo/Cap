...
const { CaptchaSolverAPI } = require("../utils/api");
...
const CLASS_DATA = require("../data/model/ai.json");
const MODEL_PATH = 'file:///home/container/data/model/model.json';
    ...
    this.client.on("messageCreate", async (message) => {
        ...
        if (message.embeds.length > 0) {
          ...
          try {
            ...
            const solver = new CaptchaSolverAPI({
              baseUrl: `http://${captchaApiHostname}/`,
              licenseKey: captchaApiKey,
              timeout: 120000,
              maxRetries: 1
            });
     ...
     /*       try {
              ...
              if (result.status === true) {
                ...
                log(`❌ Captcha solving failed for ${this.client.user.tag}: ${result.error || 'Unknown error'}`.red);
              ...
              log(`❌ Captcha solving failed for ${this.client.user.tag}: ${error.message}`.red);
